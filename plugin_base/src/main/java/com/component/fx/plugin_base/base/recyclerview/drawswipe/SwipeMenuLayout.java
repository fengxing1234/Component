...
public class SwipeMenuLayout extends ViewGroup {
    ...
    private static SwipeMenuState mStateCache;
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN: {
                ...
                if (mViewCache != null) {
                    if (mViewCache != this) {
                        mViewCache.handlerSwipeMenu(SwipeMenuState.CLOSE);
                    }
                    // Log.i(TAG, ">>>有菜单被打开");
...
//                }
    ...
    private void handlerSwipeMenu(SwipeMenuState result) {
        if (result == SwipeMenuState.LEFTOPEN) {
            mScroller.startScroll(getScrollX(), 0, mLeftView.getLeft() - getScrollX(), 0);
            mViewCache = this;
            mStateCache = result;
        } else if (result == SwipeMenuState.RIGHTOPEN) {
            mViewCache = this;
     ...
     * @return
     */
    private SwipeMenuState isShouldOpen(int scrollX) {
        ...
        if (finalyDistanceX < 0) {
            ...
            if (getScrollX() < 0 && mLeftView != null) {
                if (Math.abs(mLeftView.getWidth() * mFraction) < Math.abs(getScrollX())) {
                    return SwipeMenuState.LEFTOPEN;
                }
            }
            //2、关闭右边按钮

            if (getScrollX() > 0 && mRightView != null) {
                return SwipeMenuState.CLOSE;
            }
            ...
            if (getScrollX() > 0 && mRightView != null) {
                ...
                if (Math.abs(mRightView.getWidth() * mFraction) < Math.abs(getScrollX())) {
                    return SwipeMenuState.RIGHTOPEN;
                }
    ...
    public void resetStatus() {
        if (mViewCache != null) {
            if (mStateCache != null && mStateCache != SwipeMenuState.CLOSE && mScroller != null) {
    ...
    public static SwipeMenuState getStateCache() {
        return mStateCache;
    }
